...
const CONTENT_TYPES = ["link", "app", "file", "text", "video"] as const;
type ContentTypeKey = typeof CONTENT_TYPES[number];
...
export default function AdminScreen() {
  ...
  // ── recharge state ─────────────────────────────────────────────────────────
  type RechargeItem = { id: number; username: string; paymentMethod: string; transactionId: string; transferImageUrl: string; status: string; createdAt: string };
  ...
  const refetchLessons = async () => {
    const res = await fetch(`${apiBase}/api/admin/lessons`, { credentials: "include" });
    if (res.ok) setLessons(await res.json());
  };
  const refetchServices = async () => {
    const res = await fetch(`${apiBase}/api/admin/services`, { credentials: "include" });
  ...
  const fetchRechargeRequests = async () => {
    ...
    } catch { /* ignore */ } finally { setRechargeLoading(false); }
  };
  ...
  const fetchSentLog = async () => {
    ...
    finally { setSentLogLoading(false); }
  };
    ...
    try {
      if (tab === "tools") {
        const res = await fetch(`${apiBase}/api/tools`, {
          method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include",
          body: JSON.stringify({ title: addTitle, description: addDescription || " ", category: addCategory || "عام", color: addColor || "#d4a017", url: addUrl || null, imageUrl: addImageUrl || null, content: addContent || null, contentType: addContentType || null, isActive: addIsActive }),
        });
        ...
        const res = await fetch(`${apiBase}/api/services`, {
          method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include",
          body: JSON.stringify({ title: addTitle, description: addDescription || " ", category: addCategory || "عام", price: Number(addPrice || 0), url: addUrl || null, imageUrl: addImageUrl || null, content: addContent || null, contentType: addContentType || null, isAvailable: addIsActive }),
        });
        if (!res.ok) { Alert.alert("خطأ", "فشل الإضافة"); return; }
        ...
        const res = await fetch(`${apiBase}/api/lessons`, {
          method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include",
          body: JSON.stringify({ title: addTitle, description: addDescription || " ", category: addCategory || "عام", duration: Number(addDuration || 0), videoUrl: addVideoUrl || null, imageUrl: addImageUrl || null, content: addContent || null, contentType: addContentType || null, isPublished: addIsActive }),
        });
        if (!res.ok) { Alert.alert("خطأ", "فشل الإضافة"); return; }
  ...
  // ── pick tool file (.txt/.py/.apk) ────────────────────────────────────────
  const pickToolFile = async () => {
    if (Platform.OS === "web") return;
    try {
      const result = await DocumentPicker.getDocumentAsync({
        type: ["text/plain", "text/x-python", "application/vnd.android.package-archive", "*/*"],
      ...
      Alert.alert("خطأ", "فشل رفع الملف: " + e.message);
    } finally {
  ...
  const pickLessonVideo = async () => {
    ...
    try {
      ...
      if (asset.fileSize && asset.fileSize > 50 * 1024 * 1024) {
        Alert.alert("تنبيه", "حجم الفيديو كبير جداً (الحد الأقصى 50 ميجابايت). استخدم رابطاً خارجياً بدلاً من ذلك.");
        return;
      }
      setAddFileUploading(true);
      const base64 = await FileSystem.readAsStringAsync(asset.uri, { encoding: "base64" });
      ...
      Alert.alert("خطأ", "فشل رفع الفيديو: " + e.message);
    } finally {
      setAddFileUploading(false);
    }
  };
  ...
  const saveBanner = async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/api/banners`, {
        method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include",
        body: JSON.stringify({ imageUrl: bannerImageUrl, title: bannerTitle || null, linkUrl: bannerLinkUrl || null, sortOrder: Number(bannerOrder || 0) }),
      });
      if (!res.ok) { Alert.alert("خطأ", "فشل الإضافة"); return; }
      await fetchBanners();
    ...
    Alert.alert("تأكيد الحذف", "هل تريد حذف هذا البانر؟", [
      { text: "إلغاء", style: "cancel" },
      {
        text: "حذف", style: "destructive", onPress: async () => {
          await fetch(`${apiBase}/api/banners/${id}`, { method: "DELETE", credentials: "include" });
          await fetchBanners();
        }
      },
    ]);
  ...
  const toggleBannerActive = async (id: number, current: boolean) => {
    await fetch(`${apiBase}/api/banners/${id}`, {
      method: "PATCH", headers: { "Content-Type": "application/json" }, credentials: "include",
  ...
  // ── charge balance ─────────────────────────────────────────────────────────
  const openCharge = (userId: number, userName: string, currentBalance: string) => {
    setChargeUserId(userId);
    setChargeUserName(userName);
    setChargeAmount("");
    setChargeNote("");
    setShowCharge(true);
  };
  ...
  const saveCharge = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/users/${chargeUserId}`, {
        method: "PATCH", headers: { "Content-Type": "application/json" }, credentials: "include",
        body: JSON.stringify({ balance: newBalance }),
      });
      if (!res.ok) { Alert.alert("خطأ", "فشل شحن الرصيد"); return; }
      if (chargeNote.trim()) {
        await fetch(`${apiBase}/api/notifications/user/${chargeUserId}`, {
          method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include",
          body: JSON.stringify({ title: "تم شحن رصيدك ✅", message: chargeNote.trim() || `تمت إضافة ${added} إلى رصيدك. الرصيد الحالي: ${newBalance}`, type: "recharge_approved" }),
        });
      }
  ...
  const sendNotification = async () => {
    ...
    try {
      if (notifyUserId) {
        const res = await fetch(`${apiBase}/api/notifications/user/${notifyUserId}`, {
          method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include",
          body: JSON.stringify({ title: notifyTitle, message: notifyBody, type: "admin_message" }),
        });
        if (!res.ok) { Alert.alert("خطأ", "فشل الإرسال"); return; }
      } else {
        const res = await fetch(`${apiBase}/api/notifications/broadcast`, {
          method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include",
          body: JSON.stringify({ title: notifyTitle, message: notifyBody, type: "app_update" }),
        });
  ...
  // ── shared content type picker (reused in add/edit) ────────────────────────
    ...
    <View style={{ flexDirection: "row-reverse", flexWrap: "wrap", gap: 6 }}>
      {CONTENT_TYPES.map((ct) => (
        <TouchableOpacity key={ct} onPress={() => onChange(ct)} activeOpacity={0.8}
          ...
          <Text style={{ fontFamily: "Cairo_600SemiBold", fontSize: 12, color: value === ct ? "#0f1624" : colors.foreground }}>
            {CT_LABELS[ct]}
          </Text>
        </TouchableOpacity>
      ))}
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPad, backgroundColor: colors.card, borderBottomColor: colors.border }]}>
        <View style={styles.headerRow}>
          ...
          <TouchableOpacity
            onLongPress={startSecretHold} onPressOut={cancelSecretHold}
            onPress={() => { if (secretVisible) setTab("overview"); }}
            delayLongPress={7000} style={styles.titleWrap} activeOpacity={0.9}
          >
            <Text style={[styles.headerTitle, { color: colors.primary }]}>السوري معك</Text>
            {secretProgress > 0 && (
              <View style={[styles.holdBar, { borderColor: colors.border }]}>
                <View style={[styles.holdFill, { width: `${Math.round(secretProgress * 100)}%`, backgroundColor: colors.primary }]} />
              </View>
            )}
          </TouchableOpacity>
      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: isWeb ? 40 : 60 }]} showsVerticalScrollIndicator={false}
        ...
        {tab === "users" && (
          <View style={styles.listSection}>
            {users?.map((u) => (
              <View key={u.id} style={[styles.listItem, { backgroundColor: colors.card, borderColor: colors.border }]}>
                ...
                <View style={styles.actionRow}>
                  <TouchableOpacity onPress={() => openCharge(u.id, u.username, String(u.balance))} style={[styles.chargeBtn, { backgroundColor: colors.primary + "22", borderColor: colors.primary + "44" }]}>
                    <Feather name="plus-circle" size={16} color={colors.primary} />
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => openNotify(u.id)} style={[styles.notifyBtn, { backgroundColor: "#3b82f622", borderColor: "#3b82f644" }]}>
            ...
            {(!users || users.length === 0) && (
              <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>لا يوجد مستخدمون حتى الآن</Text>
            )}
          </View>
        )}
          ...
          <View style={{ gap: 16 }}>
            ...
            <View style={[styles.notifSection, { backgroundColor: colors.card, borderColor: "#3b82f644" }]}>
              ...
              <TouchableOpacity
                ...
                style={[styles.notifSendBtn, { backgroundColor: "#3b82f6" }]}
                activeOpacity={0.8}
              >
                  ...
                  <View style={styles.listItemMain}>
                    ...
                    {u.pushToken ? (
                      <View style={[styles.badge, { backgroundColor: "#34d39922" }]}>
                        <Text style={[styles.badgeText, { color: "#34d399" }]}>جهاز مُسجَّل</Text>
                      </View>
                    ) : (
                      <View style={[styles.badge, { backgroundColor: colors.secondary }]}>
                        <Text style={[styles.badgeText, { color: colors.mutedForeground }]}>بدون جهاز</Text>
                      </View>
                    )}
                  </View>
                  <TouchableOpacity
                    onPress={() => openNotify(u.id)}
                    style={[styles.notifyBtn, { backgroundColor: "#3b82f622", borderColor: "#3b82f644" }]}
                  >
                    <Feather name="bell" size={16} color="#60a5fa" />
                  </TouchableOpacity>
                </View>
                  ...
                  <View key={item.id} style={[styles.sentLogItem, { borderColor: colors.border }]}>
                    <View style={styles.sentLogHeader}>
                      ...
                      <View style={[styles.sentLogBadge, { backgroundColor: item.type === "app_update" ? "#3b82f622" : "#d4a01722" }]}>
                        <Text style={[styles.sentLogBadgeText, { color: item.type === "app_update" ? "#60a5fa" : "#d4a017" }]}>
                          {item.type === "app_update" ? "بث عام" : "مستخدم محدد"}
                        </Text>
                      </View>
                    </View>
                    {!!item.message && (
                      <Text style={[styles.sentLogMsg, { color: colors.mutedForeground }]} numberOfLines={2}>{item.message}</Text>
                    )}
                    <View style={styles.sentLogFooter}>
                      <Text style={[styles.sentLogMeta, { color: colors.mutedForeground }]}>
                        {new Date(item.sentAt).toLocaleDateString("ar-SA", { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                      </Text>
                      <View style={styles.sentLogRecipients}>
                        <Feather name="users" size={12} color="#94a3b8" />
                        <Text style={[styles.sentLogMeta, { color: colors.mutedForeground }]}>{item.recipientCount}</Text>
                      </View>
                    </View>
                  </View>
                ))
        ...
        {tab === "lessons" && (
          <View style={styles.listSection}>
            <TouchableOpacity onPress={openAdd} style={[styles.addBtn, { backgroundColor: colors.primary + "22", borderColor: colors.primary + "44" }]} activeOpacity={0.8}>
              ...
              <Text style={[styles.addBtnText, { color: colors.primary }]}>إضافة درس جديد</Text>
            </TouchableOpacity>
            {lessons?.map((l) => (
              <View key={l.id} style={[styles.listItem, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <View style={styles.listItemMain}>
                  ...
                  <View style={[styles.badge, { backgroundColor: l.isPublished ? "#34d39922" : colors.secondary }]}>
                    <Text style={[styles.badgeText, { color: l.isPublished ? "#34d399" : colors.mutedForeground }]}>{l.isPublished ? "منشور" : "مسودة"}</Text>
                  </View>
                </View>
                <View style={styles.actionRow}>
            ...
            {(!lessons || lessons.length === 0) && (
              <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>لا توجد دروس حتى الآن</Text>
            )}
          </View>
        )}
        ...
        {tab === "tools" && (
          <View style={styles.listSection}>
            <TouchableOpacity onPress={openAdd} style={[styles.addBtn, { backgroundColor: colors.primary + "22", borderColor: colors.primary + "44" }]} activeOpacity={0.8}>
              <Feather name="plus" size={18} color={colors.primary} />
              <Text style={[styles.addBtnText, { color: colors.primary }]}>إضافة أداة جديدة</Text>
            </TouchableOpacity>
            {tools?.map((t) => (
              <View key={t.id} style={[styles.listItem, { backgroundColor: colors.card, borderColor: colors.border }]}>
                ...
                <View style={styles.listItemMain}>
                  ...
                  <View style={[styles.badge, { backgroundColor: t.isActive ? "#34d39922" : colors.secondary }]}>
                    <Text style={[styles.badgeText, { color: t.isActive ? "#34d399" : colors.mutedForeground }]}>{t.isActive ? "نشط" : "غير نشط"}</Text>
                  </View>
                </View>
                <View style={styles.actionRow}>
            ...
            {(!tools || tools.length === 0) && (
              <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>لا توجد أدوات حتى الآن</Text>
            )}
          </View>
        )}
        ...
        {tab === "services" && (
          <View style={styles.listSection}>
            <TouchableOpacity onPress={openAdd} style={[styles.addBtn, { backgroundColor: colors.primary + "22", borderColor: colors.primary + "44" }]} activeOpacity={0.8}>
              <Feather name="plus" size={18} color={colors.primary} />
              <Text style={[styles.addBtnText, { color: colors.primary }]}>إضافة خدمة جديدة</Text>
            </TouchableOpacity>
            {services?.map((s) => (
              <View key={s.id} style={[styles.listItem, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <View style={styles.listItemMain}>
                  ...
                  <View style={[styles.badge, { backgroundColor: s.isAvailable ? "#34d39922" : colors.secondary }]}>
                    <Text style={[styles.badgeText, { color: s.isAvailable ? "#34d399" : colors.mutedForeground }]}>{s.isAvailable ? "متاح" : "غير متاح"}</Text>
                  </View>
                </View>
                <View style={styles.actionRow}>
            ...
            {(!services || services.length === 0) && (
              <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>لا توجد خدمات حتى الآن</Text>
            )}
          </View>
        )}
        ...
        {tab === "banners" && (
          <View style={styles.listSection}>
            <TouchableOpacity onPress={() => { setBannerImageUrl(""); setBannerTitle(""); setBannerLinkUrl(""); setBannerOrder("0"); setShowAddBanner(true); }}
              style={[styles.addBtn, { backgroundColor: colors.primary + "22", borderColor: colors.primary + "44" }]} activeOpacity={0.8}>
              <Feather name="plus" size={18} color={colors.primary} />
              <Text style={[styles.addBtnText, { color: colors.primary }]}>إضافة بانر جديد</Text>
            </TouchableOpacity>
              ...
              <View key={b.id} style={[styles.listItem, { backgroundColor: colors.card, borderColor: colors.border, flexDirection: "column", alignItems: "flex-end" }]}>
                ...
                <View style={{ width: "100%", flexDirection: "row-reverse", alignItems: "center", gap: 10 }}>
                  <View style={styles.listItemMain}>
                    ...
                    <View style={[styles.badge, { backgroundColor: b.isActive ? "#34d39922" : colors.secondary }]}>
                      <Text style={[styles.badgeText, { color: b.isActive ? "#34d399" : colors.mutedForeground }]}>{b.isActive ? "نشط" : "مخفي"}</Text>
                    </View>
                  </View>
                  <View style={styles.actionRow}>
                    <TouchableOpacity onPress={() => toggleBannerActive(b.id, b.isActive)} style={styles.editBtn}>
                      <Feather name={b.isActive ? "eye-off" : "eye"} size={16} color={colors.foreground} />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => deleteBanner(b.id)} style={styles.deleteBtn}>
            ...
            {!bannersLoading && banners.length === 0 && (
              <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>لا توجد بنرات حتى الآن</Text>
            )}
          </View>
        )}
        ...
        {tab === "recharge" && (
          <View style={styles.listSection}>
            <TouchableOpacity onPress={fetchRechargeRequests} style={[styles.addBtn, { backgroundColor: colors.primary + "22", borderColor: colors.primary + "44" }]} activeOpacity={0.8}>
              ...
              <Text style={[styles.addBtnText, { color: colors.primary }]}>تحديث القائمة</Text>
            </TouchableOpacity>
                ...
                <View key={r.id} style={[styles.listItem, { backgroundColor: colors.card, borderColor: colors.border, flexDirection: "column", alignItems: "flex-end", gap: 8 }]}>
                  ...
                  {isPending && (
                    <View style={{ flexDirection: "row-reverse", gap: 8, width: "100%" }}>
                      <TouchableOpacity onPress={() => { Alert.alert("تأكيد", "قبول طلب الشحن؟", [{ text: "إلغاء", style: "cancel" }, { text: "قبول", onPress: () => handleRechargeStatus(r.id, "approved") }]); }}
                        style={[styles.modalBtn, { backgroundColor: "#34d39922", flex: 1 }]}>
                        <Text style={[styles.modalBtnText, { color: "#34d399" }]}>قبول ✓</Text>
                      </TouchableOpacity>
                      <TouchableOpacity onPress={() => { Alert.alert("تأكيد", "رفض طلب الشحن؟", [{ text: "إلغاء", style: "cancel" }, { text: "رفض", style: "destructive", onPress: () => handleRechargeStatus(r.id, "rejected") }]); }}
                        style={[styles.modalBtn, { backgroundColor: "#ef444422", flex: 1 }]}>
            ...
            {!rechargeLoading && rechargeRequests.length === 0 && (
              <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>لا توجد طلبات شحن حتى الآن</Text>
            )}
          </View>
        )}
      </ScrollView>
      ...
      <EditDialog
        ...
        fields={
          <View style={{ gap: 10 }}>
            ...
            {edit.type === "lesson" && (
              <>
                ...
                <View style={{ flexDirection: "row-reverse", gap: 8, alignItems: "center" }}>
                  <TextInput value={extra2} onChangeText={setExtra2} placeholder="رابط الفيديو" placeholderTextColor={colors.mutedForeground} style={[inputStyle, { flex: 1, marginBottom: 0 }]} autoCapitalize="none" keyboardType="url" />
                  {Platform.OS !== "web" && (
                    <TouchableOpacity
                      onPress={async () => {
                        try {
                          ...
                          if (result.canceled) return;
                          const asset = result.assets[0];
                          if (asset.fileSize && asset.fileSize > 50 * 1024 * 1024) { Alert.alert("تنبيه", "الحجم كبير جداً (الحد 50MB)"); return; }
                          setAddFileUploading(true);
                          const b64 = await FileSystem.readAsStringAsync(asset.uri, { encoding: "base64" });
                          const res = await fetch(`${apiBase}/api/upload`, { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ base64: b64, mimeType: asset.mimeType ?? "video/mp4" }) });
                          const json = await res.json();
                      ...
                      style={[styles.chargeBtn, { backgroundColor: colors.primary + "22", borderColor: colors.primary + "44" }]}
                    >
            ...
            {/* shared image + content fields for non-user types */}
            {edit.type !== "user" && (
              <>
                <View style={{ flexDirection: "row-reverse", gap: 8, alignItems: "center" }}>
                  <TextInput value={imageUrl} onChangeText={setImageUrl} placeholder="رابط صورة البطاقة (اختياري)" placeholderTextColor={colors.mutedForeground} style={[inputStyle, { flex: 1, marginBottom: 0 }]} autoCapitalize="none" keyboardType="url" />
                  {Platform.OS !== "web" && (
                    <TouchableOpacity onPress={() => pickCardImage(setImageUrl)} disabled={cardImageUploading}
                      style={[styles.chargeBtn, { backgroundColor: colors.primary + "22", borderColor: colors.primary + "44", opacity: cardImageUploading ? 0.5 : 1 }]}>
      ...
      {/* ── add dialog (tools / services / lessons) ── */}
        ...
        <View style={styles.modalBackdrop}>
          <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: "center", padding: 20 }} keyboardShouldPersistTaps="handled">
            <View style={[styles.modalCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <View style={{ gap: 10 }}>
                ...
                {tab === "lessons" && (
                  <>
                    ...
                    <View style={{ flexDirection: "row-reverse", gap: 8, alignItems: "center" }}>
                      <TextInput value={addVideoUrl} onChangeText={setAddVideoUrl} placeholder="رابط الفيديو (اختياري)" placeholderTextColor={colors.mutedForeground} style={[inputStyle, { flex: 1, marginBottom: 0 }]} autoCapitalize="none" keyboardType="url" />
                      {Platform.OS !== "web" && (
                        <TouchableOpacity
                          onPress={pickLessonVideo}
                          disabled={addFileUploading}
                          style={[styles.chargeBtn, { backgroundColor: colors.primary + "22", borderColor: colors.primary + "44", opacity: addFileUploading ? 0.5 : 1 }]}
                        >
                          <Feather name={addFileUploading ? "loader" : "video"} size={18} color={colors.primary} />
                        </TouchableOpacity>
                      )}
                ...
                {tab === "tools" && (
                  <TextInput value={addColor} onChangeText={setAddColor} placeholder="اللون (مثال: #d4a017)" placeholderTextColor={colors.mutedForeground} style={inputStyle} />
                )}
                {tab === "services" && (
                  <TextInput value={addPrice} onChangeText={setAddPrice} placeholder="السعر" placeholderTextColor={colors.mutedForeground} keyboardType="numeric" style={inputStyle} />
                )}

                <View style={{ flexDirection: "row-reverse", gap: 8, alignItems: "center" }}>
                  <TextInput value={addImageUrl} onChangeText={setAddImageUrl} placeholder="رابط صورة البطاقة (اختياري)" placeholderTextColor={colors.mutedForeground} style={[inputStyle, { flex: 1, marginBottom: 0 }]} autoCapitalize="none" keyboardType="url" />
                  {Platform.OS !== "web" && (
                    <TouchableOpacity onPress={() => pickCardImage(setAddImageUrl)} disabled={cardImageUploading}
                      style={[styles.chargeBtn, { backgroundColor: colors.primary + "22", borderColor: colors.primary + "44", opacity: cardImageUploading ? 0.5 : 1 }]}>
                      <Feather name={cardImageUploading ? "loader" : "image"} size={18} color={colors.primary} />
                    </TouchableOpacity>
                  )}
                ...
                {tab === "tools" && (
                  <TextInput
                    ...
                    placeholder="رابط الأداة التفاعلية (يفتح عند الضغط على الكارت)"
                    placeholderTextColor={colors.mutedForeground}
                    style={inputStyle}
                    autoCapitalize="none"
                    keyboardType="url"
                ...
                {tab === "tools" && addContentType === "file" && Platform.OS !== "web" && (
                  <TouchableOpacity
                    onPress={pickToolFile}
                    disabled={addFileUploading}
                    style={[styles.uploadFileBtn, { backgroundColor: colors.secondary, borderColor: colors.border, opacity: addFileUploading ? 0.5 : 1 }]}
                  >
                    <Feather name={addFileUploading ? "loader" : "upload"} size={16} color={colors.primary} />
                    <Text style={[styles.uploadFileBtnText, { color: colors.primary }]}>
                      {addFileUploading ? "جاري الرفع..." : "اختر ملف (.txt / .py / .apk)"}
                    </Text>
                  </TouchableOpacity>
                )}
              ...
              <View style={styles.modalActions}>
                <TouchableOpacity onPress={() => setShowAdd(false)} style={[styles.modalBtn, { backgroundColor: colors.secondary }]}>
                  <Text style={[styles.modalBtnText, { color: colors.foreground }]}>إلغاء</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={saveAdd} style={[styles.modalBtn, { backgroundColor: colors.primary }]}>
                  <Text style={[styles.modalBtnText, { color: "#0f1624" }]}>إضافة</Text>
                </TouchableOpacity>
              </View>
            </View>
          </ScrollView>
            ...
            <View style={[styles.modalCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <View style={{ gap: 10 }}>
                <View style={{ flexDirection: "row-reverse", gap: 8, alignItems: "center" }}>
                  ...
                  {Platform.OS !== "web" && (
                    <TouchableOpacity
                      ...
                      style={[styles.chargeBtn, { backgroundColor: colors.primary + "22", borderColor: colors.primary + "44", opacity: bannerUploading ? 0.5 : 1 }]}
                    >
                ...
                {bannerImageUrl.trim().length > 0 && (
                  <Image source={{ uri: bannerImageUrl }} style={[styles.bannerThumb, { alignSelf: "center" }]} resizeMode="cover" />
                )}
              ...
              <View style={styles.modalActions}>
                <TouchableOpacity onPress={() => setShowAddBanner(false)} style={[styles.modalBtn, { backgroundColor: colors.secondary }]}>
                  <Text style={[styles.modalBtnText, { color: colors.foreground }]}>إلغاء</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={saveBanner} style={[styles.modalBtn, { backgroundColor: colors.primary }]}>
                  <Text style={[styles.modalBtnText, { color: "#0f1624" }]}>إضافة</Text>
                </TouchableOpacity>
              </View>
            </View>
      ...
      <Modal visible={showCharge} transparent animationType="fade" onRequestClose={() => setShowCharge(false)}>
        <View style={styles.modalBackdrop}>
          <View style={[styles.modalCard, { backgroundColor: colors.card, borderColor: colors.border, margin: 20 }]}>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity onPress={() => setShowCharge(false)} style={[styles.modalBtn, { backgroundColor: colors.secondary }]}>
                <Text style={[styles.modalBtnText, { color: colors.foreground }]}>إلغاء</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={saveCharge} style={[styles.modalBtn, { backgroundColor: colors.primary }]}>
                <Text style={[styles.modalBtnText, { color: "#0f1624" }]}>شحن</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      <Modal visible={showNotify} transparent animationType="fade" onRequestClose={() => setShowNotify(false)}>
        <View style={styles.modalBackdrop}>
          <View style={[styles.modalCard, { backgroundColor: colors.card, borderColor: colors.border, margin: 20 }]}>
            <Text style={[styles.modalTitle, { color: colors.foreground }]}>
              {notifyUserId ? "إرسال إشعار لمستخدم" : "إشعار لجميع المستخدمين"}
            </Text>
            <View style={{ gap: 10 }}>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity onPress={() => setShowNotify(false)} style={[styles.modalBtn, { backgroundColor: colors.secondary }]}>
                <Text style={[styles.modalBtnText, { color: colors.foreground }]}>إلغاء</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={sendNotification} style={[styles.modalBtn, { backgroundColor: "#3b82f6" }]}>
                <Text style={[styles.modalBtnText, { color: "#fff" }]}>إرسال</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
